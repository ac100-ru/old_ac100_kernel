...
void tegra_pinmux_config_pinmux_table(struct tegra_pingroup_config *config,
					int len, bool is_set)
{
	int i;
	int err;
...
void tegra_pinmux_config_tristate_table(struct tegra_pingroup_config *config,
...
void tegra_pinmux_set_vddio_tristate(tegra_vddio_t vddio, tegra_tristate_t tristate)
{
	int err;
	int pg;
	for (pg = TEGRA_PINGROUP_ATA; pg < TEGRA_MAX_PINGROUP; ++pg) {
		if (pingroups[pg].vddio == vddio) {
			if (pingroups[pg].tri_reg != REG_N) {
				err = tegra_pinmux_set_tristate(pg, tristate);
				if (err < 0)
					pr_err("pinmux: can't set pingroup %s tristate"
						" to %s: %d\n",	pingroup_name(pg),
						tri_name(tristate), err);
...
void tegra_pinmux_config_pullupdown_table(struct tegra_pingroup_config *config,
