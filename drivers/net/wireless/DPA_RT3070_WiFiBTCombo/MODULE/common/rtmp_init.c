...
#else
	{PBF_MAX_PCNT,			0x1F3F6F6F}, 	//0x1F3f7f9f},		//Jan, 2006/04/20
#endif // INF_AMAZON_SE //
	...
	IN	PSTRING			mac_addr)
{
	...
	if (((value <= REGION_MAXIMUM_BG_BAND) && (value2 <= REGION_MAXIMUM_A_BAND)) || (value == REGION_32_BG_BAND) )
	{
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (IS_RT3070(pAd) || IS_RT3071(pAd) || IS_RT3390(pAd))
	{
		// RF_BLOCK_en
		...
		// Idoh
		RT30xxReadRFRegister(pAd, RF_R09, &rfreg);
		rfreg &= (~0x0e);
		RT30xxWriteRFRegister(pAd, RF_R09, rfreg);

		// RF R21
		RT30xxReadRFRegister(pAd, RF_R21, &rfreg);
		rfreg &= (~0x80);
		RT30xxWriteRFRegister(pAd, RF_R21, rfreg);
	}
	...
	{
		if (!IS_RT3572(pAd) && !IS_RT3390(pAd))
		{
			// LDORF_VC
			RT30xxReadRFRegister(pAd, RF_R27, &rfreg);
			rfreg |= 0x77;
			RT30xxWriteRFRegister(pAd, RF_R27, rfreg);
		}

		RTUSBReadMACRegister(pAd, LDO_CFG0, &MACValue);
		MACValue |= 0x1D000000;
		RTUSBWriteMACRegister(pAd, LDO_CFG0, MACValue);		
	}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_CPU_SUSPEND))
	{
		...
		pm_usage_cnt = atomic_read(&pObj->intf->pm_usage_cnt);	
#else
		pm_usage_cnt = pObj->intf->pm_usage_cnt;
#endif
		...
		if(pm_usage_cnt == 0)
		{
...
#endif
			{
				...
				if (res < 0)
				{
					DBGPRINT(RT_DEBUG_ERROR, ("AsicRadioOn autopm_resume fail ------\n"));
					return;
				}
			}
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,33)
			...
			{
				DBGPRINT(RT_DEBUG_TRACE, ("AsicRadioOn: fRTMP_ADAPTER_SUSPEND\n"));
	...
	{
		DBGPRINT(RT_DEBUG_TRACE, ("AsicRadioOn: fRTMP_ADAPTER_CPU_SUSPEND\n"));
		return;
	}
	...
	IN RTMP_INF_TYPE infType)
{
	...
	RTMP_SEM_EVENT_INIT(&(pAd->UsbVendorReq_semaphore2));
...
EXPORT_SYMBOL(AsicRadioOff);
EXPORT_SYMBOL(AsicRadioOn);
